...
module.exports = {
  ...
  getRoomByCheckOutId: async (idRoom) => {
    const rows = await db.load(
      `SELECT rr.id as rentReceiptId, rr.price, rt.priceHour, dateIn, dateOut FROM rentreceipt rr, rentreceiptdetail rrd, Room r, RoomType rt WHERE r.id = rrd.idRoom and rrd.idRentReceipt=rr.id and r.id=rrd.idRoom and r.idType=rt.id and r.id = ${idRoom}`
    );
